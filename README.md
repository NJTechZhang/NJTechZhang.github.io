...
├── papers/                     # 论文 AI 中文导读（每篇论文一个页面）
│   ├── index.html              # 按年份索引的总览页（独立页面，主页不放入口，可经 /papers/ 直接访问）
...
「发表论文」列表中，**每篇论文下方的「AI中文导读」按钮**即该论文中文导读页的唯一入口。每篇导读包含：
...
共 **22 篇**。另有按年份索引的总览页 `papers/index.html`（主页有意不放入口，可通过 `站点地址/papers/` 直接访问）。
...
|---|---|---|
| 教育背景 / 工作经历 | `TODO: 教育背景、工作经历确认后在此补充` | 个人简介卡片内，格式已给出示例 |
| 个人照片 | 替换 `assets/img/avatar.jpg` | 建议竖版或正方形，500px 以上 |

以下信息已按已发表论文的作者简介确认并填写：

| 项目 | 内容 | 依据 |
|---|---|---|
| 职称 | 讲师（Lecturer）、博士 | 《复杂城市动态图卷积网络三维场景语义分割法》测绘学报 2023 作者简介；《复杂城市场景三维网格模型智能解译技术综述》中国图象图形学报 2025 作者简介 |
| 研究方向 | 遥感三维智能生成与解译、遥感信息智能处理 | 同上（原文表述） |
| 邮箱 | zrt@njtech.edu.cn | 论文通讯邮箱、学院公开信息 |
| 单位 | 南京工业大学测绘科学与技术学院 | — |
